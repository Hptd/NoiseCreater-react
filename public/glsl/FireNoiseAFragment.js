...
float fbm(vec2 n) {
    ...
    for (int i = 0; i < detail; i++) { // 4 细节程度 int(1 - 10)
...
void main(){
	// vec2 fragCoord = vUv;
  vec2 fragCoord = vec2((vUv.x+uvMoveX)*uvScaleX, (vUv.y+uvMoveY)*uvScaleY) * (uvScale);
    ...
    vec2 speedMain = vec2(hue, saturate); // 速度 (0 - 2.)
    float shift = onlyBri; // 位移偏转 (0 - 1.6)

    vec2 p = fragCoord.xy * 10.;
    float q = fbm(p - iTime * speed); // 0.1 速度 (0 - 2)
    vec2 r = vec2(fbm(p + q + iTime * speedMain.x - p.x - p.y), fbm(p + q - iTime * speedMain.y));
