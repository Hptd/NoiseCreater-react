...
// float widthRange = maxSize; // 方块最大尺寸 (0.002 - 0.05)
float scale = 20.0;
float alpha = 1.0;
...
float grid(vec2 r, float scale)
{
    ...
    for(int i = -1; i < 2; ++i)
    {
        for(float j = -1.0; j < 2.0; ++j)
        {
            ...
            //开启旋转
            if(rotate){angle = 3.0*(rand(vec2(rr + 0.1)) - 1.0)*iTime/2.0;}

            widthPhase = rand(vec2(rr + 0.2));
            widthFreq = 5.0*rand(vec2(rr + 0.3));
            squareWidth = rand(vec2(rr))*scale*(maxSize + maxSize*sin(iTime*widthFreq + widthPhase));
