...
uniform float iTime;
uniform float noiseOnlyBright;
uniform float noiseOnlyContrast;
uniform int noiseSubdivide;
uniform float noiseCellScale;
uniform float noiseWhiteScale;
...
//  float ov(vec2 p) {
	 float v = noiseOnlyBright; // 亮度
	 float a = noiseOnlyContrast; // 对比度
	 for (int i = 0;i<noiseSubdivide;i++) { // 3 => 噪波细分
		 v+= voronoi(p, iTime)*a;
		 p*=noiseCellScale; // cell 大小
		 a*=noiseWhiteScale; // 白色区域大小
	 }
	 return v;
 }
